...
pub enum ProposalKind {
   ...
   ChangeConfig { config: Config }, //council members
...
impl Contract {
    ...
    pub(crate) fn internal_create_hackathon_proposal(&mut self, info : &ProposalKind:: Hackathon){
    ) -> Promise
    match &proposal.kind {
        ProposalKind::Hackathon { .. } => {
    ...
    pub(crate) fn internal_create_meme_contest_proposal(&mut self, info : &ProposalKind:: MemeContest){
    ) -> Promise
    match &proposal.kind {
        ProposalKind::MemeContest { .. } => {
            let id = self.last_proposal_id;
            self.proposals.insert(
                id + 1,
